...
class PurchaseController extends Controller
{
    ...
    public function store(StorePurchaseRequest $request)
    {
        DB::transaction(function () use ($request) {
            ...
            $tax_percentage = $request->tax_percentage ?? null;
            $shipping_amount = $request->shipping_amount ? $request->shipping_amount * 100 : null;
            $tax_amount = Cart::instance('purchase')->tax() * 100;
            ...
            // }
            foreach (Cart::instance('purchase')->content() as $cart_item) {
                $product_tax_amount = $cart_item->options->product_tax ?? null;
                ...
                PurchaseDetail::create([
                    ...
                    'product_discount_amount' => $cart_item->options->product_discount * 100,
                    'product_discount_type' => $cart_item->options->product_discount_type,
                    'product_tax_amount' => $product_tax_amount ? $product_tax_amount * 100 : null,
                ]);
